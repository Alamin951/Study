...
### Some meta-annotation:
	@Target: Describes the targets to which an annotation can be applied; this directly corresponds to the nine contexts above. it's mainly used to refer or target another anotation.
...
## Thread-safety: 
	...
	-> by defining the fields private we can make their fields thread-local.
...
### Future object
 In java when we move to another thread  and the previous thread is executing some tast now if we want to keep a track or tracck its pogress then we can use future objest to let us know when its done
